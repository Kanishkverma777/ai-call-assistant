...
Rules:
- intent MUST be one of: pricing_objection, competitor_comparison, timeline_pushback, feature_inquiry, security_concern, complaint, purchase_intent, general_interest, other
...
def decide_action(intent_data: IntentOutput) -> str:
    ...
    if intent_data.intent == "competitor_comparison":
        return "COMPETITIVE_PLAY: Highlight unique differentiators and battlecards."
        
    if intent_data.intent == "timeline_pushback":
        return "URGENCY_CHECK: Offer accelerated onboarding or time-sensitive incentives."
        
    if intent_data.intent == "feature_inquiry":
        return "PRODUCT_DEEP_DIVE: Provide feature specifics and offer a demo."
        
    if intent_data.intent == "security_concern":
        return "TRUST_BUILD: Send compliance docs (SOC2, ISO) and explain architecture."
    ...
    if intent_data.intent == "general_interest":
        return "INTERESTED: Maintain active dialog and uncover specific pain points."
    
    return "GENERAL: Maintain dialogue."
...
# MAIN APP
# ============================================================================
st.markdown('<h1 style="font-size: 5rem; font-weight: 700; margin-bottom: -1rem; color: #E2E8F0; letter-spacing: 0.05em;">VAANI</h1>', unsafe_allow_html=True)
st.markdown("*Real-time voice intelligence and intent analysis for sales calls.*")
st.markdown("---")
...
with tab2:
    ...
    with col2:
        ...
        **competitor_comparison**
        Customer asks about competing tools

        **timeline_pushback**
        Customer delaying the purchase
        
        **feature_inquiry**
        Customer asking product capabilities
        
        **security_concern**
        Customer worried about privacy or data
